...
function setWord() {
    ...
    if (cuenta<=9){ //Se hace para 10 palabras, pasadas las 10 palabras se acaba el reto.
      cuenta++;
      ...
      document.getElementById("el-btn").onclick = function(){
        if (this.id == $goodBtn[0].id) {
            modify_qty(1,1);
            setWord();
          }
      ...
      document.getElementById("el-btn").setAttribute('disabled','disabled');
      document.getElementById("la-btn").setAttribute('disabled','disabled');
      document.getElementById("test-word").style.visibility = "hidden";
    }
}
