	...
	render: function render() {
		return React.createElement(
			...
			this.props.rowArr.map(function (cell, index) {
				return React.createElement(Cell, { rowIndex: this.props.rowIndex, index: index, cell: cell, handlePieceClick: this.props.handlePieceClick });
			}, this)
		);
	}
});
...
var Statistics = React.createClass({
	...
	render: function render() {
		return React.createElement(
			...
			React.createElement(
				"div",
				{ className: "half", style: { color: '#9577cf' } },
				"Blancas(J1):",
				React.createElement("br", null),
				(this.props.board.map(function (row) {
					return row.join('');
				}).join('').match(/r/g) || []).length,
				" Soldiers",
				React.createElement("br", null),
				(this.props.board.map(function (row) {
					return row.join('');
			...
			React.createElement(
				...
				(this.props.board.map(function (row) {
					return row.join('');
				}).join('').match(/b/g) || []).length,
				" Soldiers",
				React.createElement("br", null),
				(this.props.board.map(function (row) {
					return row.join('');
				}).join('').match(/b\sk/g) || []).length,
				" Kings"
			)
		);
	}
...
var Popup = React.createClass({
	...
	render: function render() {
		if (this.props.shown) {
			return React.createElement(
				"div",
				{ className: "pop", onClick: this.props.close },
				React.createElement(
					"div",
					{ className: "internal" },
					this.props.copy,
					React.createElement(
						"button",
						{ onClick: this.props.close, className: "close" },
						"x"
					)
				)
			);
...
var GameBoard = React.createClass({
	...
	render: function render() {
		...
		return React.createElement(
			"div",
			{ className: "container_damas" },
			React.createElement(
				"div",
				{ className: 'board ' + this.state.activePlayer },
				this.state.board.map(function (row, index) {
					return React.createElement(Row, { rowArr: row, handlePieceClick: this.handlePieceClick.bind(this), rowIndex: index });
				}, this)
			),
	...
	/*		React.createElement(
			...
			React.createElement(Statistics, { board: this.state.board }),
		//	React.createElement(Popup, { shown: this.state.popShown, close: this.aboutPopClose, copy: "a"})
		);
	},
	aboutPopOpen: function aboutPopOpen(e) {
		this.setState({ popShown: true });
	},
	aboutPopClose: function aboutPopClose(e) {
		this.setState({ popShown: false });
	},
	handlePieceClick: function handlePieceClick(e) {
	...
	executeMove: function executeMove(rowIndex, cellIndex, board, activePlayer) {
		var activePiece;
		...
		if ((typeof deletions === "undefined" ? "undefined" : _typeof(deletions)) !== undefined && deletions !== null && deletions.length > 0) {
	...
	highlightPossibleMoves: function highlightPossibleMoves(rowIndex, cellIndex) {
	...
	findAllPossibleMoves: function findAllPossibleMoves(rowIndex, cellIndex, board, activePlayer) {
		var possibleMoves = [];
	...
	findAllJumps: function findAllJumps(sourceRowIndex, sourceCellIndex, board, directionOfMotion, possibleJumps, wouldDelete, isKing, activePlayer) {
		//jump moves
	...
	winDetection: function winDetection(board, activePlayer) {
	...
	aiBranch: function aiBranch(hypotheticalBoard, activePlayer, depth) {
		...
		output = output.sort(function (a, b) {
			if (a.score > b.score) return -1;if (a.score < b.score) return 1;return 0;
		});
		return output;
	}
});
